...
function requestAcceptanceStatus(payload = {}) {
  ...
  const direct = normalized(payload.job_acceptance_status);
  ...
  const flagDirect = normalized(flagValue(flags, "rules_acceptance_"));
  if (["quote_eligible", "evidence_before_quote", "founder_review", "refuse"].includes(flagDirect)) return flagDirect;
