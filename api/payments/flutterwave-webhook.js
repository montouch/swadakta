...
async function updateRequestFromFlutterwave(eventData, verifiedData) {
  ...
  const request = requestCode ? null : await findRequestByReference(transactionRef || flutterwaveRef || transactionId);
  ...
  const updatePayload = {
    ...
    release_notes: `Flutterwave webhook and transaction verification confirmed ${currency || "provider"} payment. Founder/admin must still review milestone proof before any receiver release.`,
  };
