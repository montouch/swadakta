...
- Before a retry push, run `scripts/deployment-state.mjs` with the bundled Node runtime to confirm whether production is current, stale, failed, pending, or rate-limited.
...
- `scripts/check-launch-ready.mjs https://swadakta.com` passes before demos; it includes deployment freshness, the Supabase schema contract, production health, and visual fit. For live table/RLS/RPC/storage confirmation, run `supabase/live_contract_check.sql` in Supabase SQL editor or through the Supabase MCP SQL tool. Run the launch verifier with `--strict-auth` and shell-only E2E credentials when user/admin sign-in proof is required.
- Keep strict-auth credentials local-only: do not paste E2E passwords into chat, committed docs, `.env.example`, screenshots, or one-line commands that will be saved in shell history. Use an ignored local env file or private shell session, and run `scripts/check-e2e-credential-boundary.mjs` before paid-pilot rehearsal.
- Before any paid-pilot rehearsal, run `scripts/check-launch-ready.mjs https://swadakta.com --paid-pilot`. This fails closed unless auth E2E credentials are present, visual fit is run, founder evidence controls pass, and the live readiness summary is not in a paid-launch-blocked state.
...
- Account-level provider ID evidence syncs to receiver applications using the same email, but it does not auto-vet the receiver; vetting still requires Swadakta review of coverage, proof standards, provenance, and safety.
